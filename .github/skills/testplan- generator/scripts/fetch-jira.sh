...
# Needs env: JIRA_BASE_URL (e.g. https://yourco.atlassian.net)
# ponytail: curl + jq, no SDK. Add pagination/attachments only
set -euo pipefail
...
curl -sS -u "${JIRA_EMAIL}:${JIRA_TOKEN}" \
  ...
  "${JIRA_BASE_URL}/rest/api/3/issue/${KEY}?fields=summary,desc" |
jq '{
  key: .key,
